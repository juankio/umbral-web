<template>
  <section class="border-t border-neutral-200 bg-white py-16 lg:py-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <NuxtLink
          ...
          :key="rel.slug"
          :to="`/obras/${rel.slug}`"
          class="group bg-white border border-neutral-200 overflow-hidden hover:border-neutral-950 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 ease-out flex flex-col justify-between"
        >
          <div class="relative aspect-square bg-neutral-100 overflow-hidden">
            <img
              :src="rel.heroImage"
              :alt="rel.title"
              class="w-full h-full object-cover grayscale-[15%] group-hover:grayscale-0 scale-100 group-hover:scale-105 transition-all duration-500 ease-out will-change-transform"
              loading="lazy"
            />
            <div class="absolute top-2 left-2 transition-transform duration-300 group-hover:-translate-y-0.5">
              <span class="px-2 py-0.5 text-[10px] font-mono uppercase tracking-widest bg-neutral-950/90 text-white backdrop-blur-sm border border-neutral-800">
                {{ rel.category }}
              </span>
            </div>
            <div class="absolute top-2 right-2">
              <span class="px-2 py-0.5 text-[10px] font-mono bg-white/95 text-neutral-950 border border-neutral-200 font-bold">
                {{ rel.price }}
              </span>
            </div>
          </div>
          ...
          <div class="p-4 border-t border-neutral-100 flex-1 flex flex-col justify-between bg-white">
            <div>
              <div class="flex items-baseline justify-between">
                ...
                <span class="font-mono text-xs text-neutral-400">{{ rel.year }}</span>
              </div>
              <p class="font-sans text-[11px] text-neutral-500 uppercase tracking-wider mt-1 truncate">
                {{ rel.designers.join(', ') }}
              </p>
            </div>

            <div class="pt-3 mt-3 border-t border-neutral-100 flex items-center justify-between font-mono text-[11px] text-neutral-400">
              <span>{{ rel.dimensions }}</span>
              <span class="text-neutral-950 flex items-center gap-1 font-barlow font-bold text-xs uppercase tracking-wider group-hover:translate-x-1 transition-transform duration-200">
                Ver Obra →
              </span>
            </div>
          </div>
